/* t69: a 2-byte store of identical pointer bytes must still remove the
 * stale pointer shadow.
 *
 * Proof of removal (not byte change): the store writes the low 16 bits
...
int main(void) {
    ...
    free(p);
    __asm__ volatile("movw %w1, %0"
                     : "=m"(*(unsigned short *)&slot)
                     : "r"((unsigned short)untagged)
                     : "memory");
    char *q;
    __asm__ volatile("movq %1, %0" : "=r"(q) : "m"(slot) : "memory");
    q[0] = 1; /* must NOT be UAF: the 2-byte store removed the shadow */
    return 0;
}
