/* t68: a 1-byte store of identical pointer bytes must still remove the
 * stale pointer shadow.
 *
 * Proof of removal (not byte change): the store writes the low byte of
 * the freed pointer back unchanged, then an 8-byte reload of the slot
...
int main(void) {
    ...
    free(p);
    __asm__ volatile("movb %b1, %0"
                     : "=m"(*(unsigned char *)&slot)
                     : "r"((unsigned char)untagged)
                     : "memory");
    char *q;
    __asm__ volatile("movq %1, %0" : "=r"(q) : "m"(slot) : "memory");
    q[0] = 1; /* must NOT be UAF: the 1-byte store removed the shadow */
    return 0;
}
